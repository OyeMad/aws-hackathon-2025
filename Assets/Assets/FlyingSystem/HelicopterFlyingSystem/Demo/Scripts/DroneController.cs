...
public class DroneController : MonoBehaviour
{
    ...
    public void CameraControlLogic(float movementValueX, float movementValueY )
    {
        springArmTransform.rotation = Quaternion.Euler(springArmTransform.rotation.eulerAngles.x - movementValueY * cameraSpeed * Time.deltaTime, springArmTransform.rotation.eulerAngles.y + movementValueX * cameraSpeed * Time.deltaTime, 0.0f);
    }
    ...
    void MobileCameraControlLogic()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            {
                if (Input.GetTouch(i).position.x > screenCenterX && Input.GetTouch(i).phase == UnityEngine.TouchPhase.Moved)
                {
    ...
    void PCInputControlLogic()
    {
        ...
        {
            yAxis = 1;
            accumulatedDeltaMousePositionX += Mathf.Clamp(xAxis , -1.0f, 1.0f);
            accumulatedDeltaMousePositionY += Mathf.Clamp(yAxis , -1.0f, 1.0f);
        ...
        if (Input.GetMouseButtonUp(0) && Mathf.Approximately( v , 0.0f) )
        {
            draggingMouse = false;
